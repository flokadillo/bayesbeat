...
bpath = 'E:\UPFWork\PhD\Code\BayesResults\CMCMDa_small\HMM_viterbi_HMM_kChapu\';
...
for ex = 1:nExp
    ...
    for k = 1:length(oFiles)
        ...
        % Read output meter
        fpm = fopen(fullfile(filepath, [fname{k} '.meter']),'rt');
        temp = textscan(fpm,'%s');
        res(k).meter = temp{1}{1};
        fclose(fpm);
        % Read output rhythm/s
        fpr = fopen(fullfile(filepath, [fname{k} '.rhythm']),'rt');
        temp = textscan(fpr,'%s');
        res(k).rhythm = [temp{1}{:}];
        fclose(fpr);
