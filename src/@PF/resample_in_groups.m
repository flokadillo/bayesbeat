function [outIndex, outWeights, groups_new] = resample_in_groups(groups, weights, n_max_clusters, warp_fun)
...
parts_per_group = diff(round(linspace(0, length(weights), n_groups+1)));%targeted number of particles per cluster
...
if exist('warp_fun', 'var')
    ...
    outIndex = PF.resampleSystematicInGroups( w_warped_per_group, ...
        parts_per_group);
    ...
    outIndex = PF.resampleSystematicInGroups(w_norm_per_group, ...
        parts_per_group);
    outIndex = id_per_group(outIndex);
    groups_new = groups(outIndex);
