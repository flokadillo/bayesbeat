...
exptName = 'PF_section_AMPF_Full_NoHop_1500pp';
...
for t = 1:length(talaName)
    ...
    for ex = 1:nExp
        ...
        fp = fopen([talabasepath exptName '_' talaName{t} '_' num2str(ex) '_allResults.txt'], 'wt');
        fprintf(fp, '%s\n', [colHead1 colHead2]);
        for k = 1:length(filelist)
        ...
        sMat = [];
        bMat = [];
        samaResults(t).expt(ex).allres = res;
        samaResults(t).expt(ex).op = s;
        beatResults(t).expt(ex).op = b;
...
for m = 1:2
    ...
    for t = 1:length(talaName)
        for ex = 1:nExp
            % f-Meas
            fMeasVal = samaResults(t).expt(ex).op.fMeas';
            fMeas(t+1,ex) = nanmean(fMeasVal);
            fMeasAcc(:,ex) = fMeasVal;
            %
            precVal = samaResults(t).expt(ex).op.precision';
            prec(t+1,ex) = nanmean(precVal);
            precAcc(:,ex) = precVal;
            %
            recallVal = samaResults(t).expt(ex).op.recall';
            recall(t+1,ex) = nanmean(recallVal);
            recallAcc(:,ex) = recallVal;
            %
            cmltVal = samaResults(t).expt(ex).op.cmlT';
            cmlt(t+1,ex) = nanmean(cmltVal);
            cmltAcc(:,ex) = cmltVal;
            %
            amltVal = samaResults(t).expt(ex).op.amlT';
            amlt(t+1,ex) = nanmean(amltVal);
            amltAcc(:,ex) = amltVal;
            %
            infoGainVal = samaResults(t).expt(ex).op.infoGain';
    ...
    zz = zeros(1,nExp);
    opCol = nanmean([fMeas; zz; prec; zz; recall; zz; cmlt; zz; amlt; zz; infoGain; zz; zz],2);
    opFullCol = [opFullCol; opCol];
    clear opCol;
end
dlmwrite([bpath exptName filesep exptName '_opCol.txt'], opFullCol, 'precision', '%.2f');
