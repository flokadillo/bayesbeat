...
%                 dlmwrite(obj.exclude_songs_fln, find(exclude_song_ids));
        ...
        function [ca_fln, clust_trans_fln] = do_clustering(obj, params, bad_meters)
...
%             [cidx, ctrs] = kmeans(S, n_clusters, 'Distance', 'sqEuclidean', 'Replicates', 5, 'Options', opts);
            ...
            if params.fig_store_flag
                ...
                outfile = fullfile(params.results_path, ['patterns-', obj.dataset, '-kmeans-', type, '-', num2str(n_clusters), '.png']);
...
%             fprintf('writing %i bar-cluster assignments to %s\n', length(obj.bar_2_cluster), obj.clusters_fln);
            ...
            for km = 1:size(rhythm2meter,1)
                meterName{km} = params.meterNames{(params.meters(1,:) == rhythm2meter(km,1))...
                    & (params.meters(2,:) == rhythm2meter(km,2))};
            end
            for i = 1:n_clusters
                rhythm_names{i} = [obj.dataset, '-', meterName{i}, '_', num2str(i)];
            end
            ...
            clust_trans_fln = compute_cluster_transitions(obj);
        end
