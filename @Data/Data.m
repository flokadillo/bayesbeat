    ...
    methods
        ...
        function obj = Data(lab_fln, train)
            ...
            for i_file = 1:length(obj.file_list)
                if strcmp(obj.file_list{i_file}(1), '~') || strcmp(obj.file_list{i_file}(1), filesep) || strcmp(obj.file_list{i_file}(2), ':')  % For windows
...
%                 tempo_fln = fullfile(strrep(fpath, 'audio', 'annotations/bpm'), [fname, '.bpm']);
                ...
                % Use all styleIDs present in the file, and assign tempo
                % values to each: CAREFULL!!!!!
                styleId = unique(styleId);
                if ~isempty(styleId)
